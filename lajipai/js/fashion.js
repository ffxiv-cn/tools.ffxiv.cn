function fashion() {
    ...
    $(function () {
        ...
        , '<ul id="page_item"><li style="width:560px;" id="page_item_left"><div style="width: 100%;" id="pagenum"></div><ul style="top:0px;" id="fashion"></ul></li><li style="padding-left: 10px;padding-top: 10px;width:430px;" id="page_item_right"></li></ul>'
        );
        $('#page_itemtop').append(
        '<li class="back"><a onclick="back()"><img src="image/返回.png"></a></li>'
        , '<li style="width: 150px;"><a><img src="image/时尚配件.png"><div></div></a><p>时尚配件</p></li>'
    );
        $.ajax({
        ...
        var insert = '';
        $.ajax({
            url: './csv/fashion.csv?' + window._ver,
            success: function (data) {
                ...
                for (var i = 1; i < csvList.length; i++) {
                    if (i % 100 == 1) {
                        insert = '<li><a class="btn" onclick="fashionexplain(' + i + ')" target="_blank"><img src="image/fashion/' + csvList[i][1] + '.png" onload="image(this)">';
                        csvList[i][5] == "0" ? insert += '<div class="bd"></div></a></li>' : insert += '<div class="bd"><img src="image/logo/lv' + csvList[i][5] + '.png"></div></a></li>';
                    }
                    else {
                        insert += '<li><a class="btn" onclick="fashionexplain(' + i + ')" target="_blank"><img src="image/fashion/' + csvList[i][1] + '.png" onload="image(this)">';
                        csvList[i][5] == "0" ? insert += '<div class="bd"></div></a></li>' : insert += '<div class="bd"><img src="image/logo/lv' + csvList[i][5] + '.png"></div></a></li>';
...
function fashionexplain(i) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            csvList = $.csv()(data);
            insert += '<img style="float:left;width:192px;height:192px;margin-right: 210px;" src=image/fashion/' + csvList[i][0] + '.png onerror=this.src="image/chongwuzuoqi/068500.tex.png" onload="image(this)">';
